...
2. **Asr is not the whole fiqh profile.** Chapter 18 corrects simplified earlier Ja‘fari/shadow claims. Current selectors explicitly say “Asr convention.” Six opt-in named Sunni window accounts now supply school-specific boundaries; full Sunni subwindows, shafaq opinions and shared-interval Ja‘fari semantics remain deferred (Chapters 14, 18, 31, 33).
...
| FIQ-001–012 | Asr-only timetable choices plus six independent named Sunni window accounts; raw/displayed endpoints, preferred/ordinary/disliked/necessity fields, next-day dependencies, partial unavailability and ordering checks | Draft mappings need qualified review; observational and duration-dependent signs stay unresolved; complete shared intervals and exception rules remain unimplemented |
...
The first-third calculation uses sunset to next dawn, separate from the pre-existing sunset-to-sunrise midpoint. Next-day resolution uses the civil date and embedded timezone rules. Range-edge failures become unavailable next-day boundaries, not failures of otherwise available current-day events. Invalid order is retained and flagged rather than clamped; estimates keep their status. Timetable offsets are never legal-boundary adjustments. Dhuhr/Maghrib preferred cutoff fields and choice/necessity fields remain null (unspecified). Fajr’s named brightness endpoint is explicitly unresolved; Asr’s preferred endpoint is calculated at two shadows.
...
### Preferred-time extension (window profile revision 2)

The [Shafi‘i Asr answer](https://seekersguidance.org/answers/prayer-shafii-fiqh/following-a-madhhab-and-asr-time/) supplies the two-shadow preferred endpoint. The core solves this separately from the one-shadow window start and any timetable Asr selection, with raw/displayed provenance. A trigonometric inverse test checks the additional shadow ratio across three locations/seasons, plus a missing polar event and rounding isolation.

The [Fajr answer](https://seekersguidance.org/answers/shafii-fiqh/what-is-correct-time-for-fajr-and-sunrise/) names increasing daylight as the preferred endpoint. `daylight_brightness` retains this criterion but returns `DAYLIGHT_BRIGHTNESS_NOT_MODELED`; no arbitrary angle, sunrise substitution or fixed-minute cutoff is invented. Available outer endpoints survive and the window reports incomplete.

[The Reliance translation, f2.2](https://www.talibeilm.net/uploads/4/7/1/3/4713847/reliance_of_the_traveller_umdat_al-salik.pdf#page=47) supports prompt prayer with necessary preparation. This is shown for Dhuhr and Maghrib as qualitative guidance, not a numerical deadline. The narrow Maghrib view in f2.1 is not mixed into the adopted extended outer window. Preferred guidance is embedded in every definition/result, translated in the app, and included in the profile fingerprint. The profile ID advances to `windows.shafii@2`; the request selection remains backward compatible.


## Multi-school window catalog (engine/schema 0.3.0)

Five additional named definitions extend the existing Shafi‘i account: two Hanafi, one Maliki and two Hanbali accounts. The browser selector and request schema expose all six, preserving old request defaults. Each definition supplies its own name, summary, source titles and guidance, eliminating Shafi‘i-specific assumptions in the view. The fingerprint includes the complete window catalog.

New typed criteria include half of the sunset-to-dawn night and explicitly unresolved white twilight, yellowing, stars and prayer-duration dependencies. `preferred_from`, `disliked_after` and `necessity_from` complement existing end fields. The view distinguishes ordinary time from conditional necessity time, and does not equate unknown observations with numerical shadow thresholds. Available boundaries are checked against each other even if an outer endpoint is missing.

Sources: [Hanafi guidance](https://seekersguidance.org/answers/hanafi-fiqh/what-are-the-times-for-each-prayer-that-a-prayer-is-valid-in/), [half-night discussion](https://seekersguidance.org/answers/hanafi-fiqh/delaying-isha-until-shortly-before-fajr/), [twilight opinions](https://daruliftaa.com/salat-prayer/times-for-eisha-fajr-prayers-at-extreme-latitudes/), [Risalah and commentary](https://www.iium.edu.my/deed/lawbase/risalah_maliki/book08.html), [Umdat al-Fiqh](https://islamland.com/uploads/books/commentary-on-umdat-alfiqh-reliable-source-of-fiqh_eng.pdf#page=126), [the named Hanbali alternative](https://islamqa.org/hanbali/thehanbalimadhhab/118344/).

Validation covers distinct school/opinion outputs, raw offset isolation, fractions across both DST transitions, conditional and reserved endpoints, polar partial availability, schema/catalog agreement, every profile/method in native-WASM parity and German/Arabic rendering of every account. This is still a partial domain model: it does not decide personal excuses or implement full shared-interval ordering and reserved-duration arithmetic. Ja‘fari rules are not inferred from these Sunni accounts.
