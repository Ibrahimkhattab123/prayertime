...
2. **Asr is not the whole fiqh profile.** Chapter 18 corrects simplified earlier Ja‘fari/shadow claims. Current selectors explicitly say “Asr convention.” An opt-in draft Shafi‘i window profile now supplies outer boundaries, Asr’s two-shadow and Isha’s first-third preferred endpoints; full Sunni subwindows, shafaq opinions and shared-interval Ja‘fari semantics remain deferred (Chapters 14, 18, 31, 33).
...
| FIQ-001–012 | Asr-only timetable choices plus an independent opt-in Shafi‘i window definition; raw/displayed endpoints, Asr two-shadow preference, Fajr brightness criterion (unresolved), early-performance guidance, Isha first third and next-dawn end, partial unavailability and ordering checks | Draft angle mappings need qualified review; other preferred/choice/necessity boundaries and shared intervals remain unspecified |
