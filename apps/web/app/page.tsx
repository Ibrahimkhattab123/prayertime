...
import { hijriDateLabel as formatHijriDate } from '@/lib/hijri';
        ...
        <div className="explanation-body">
          ...
          {p.fallback && (
            <p className="estimate-note">
              ...
              event is absent ({p.fallback.primary_failure}). The reference
              night lasts{' '}
              {(
          ...
          {p.unavailable_reason && (
            <p>
              ...
              {p.unavailable_reason.replaceAll('_', ' ').toLowerCase()}. Choose
              an explicit estimation policy for missing twilight. Basic night
...
export default function Home() {
  ...
  const dateLabel = (date: string) =>
    formatDateLabel(date, localeFor(language));
  const hijriDateLabel = (date: string) =>
    formatHijriDate(date, localeFor(language));
  const [request, setRequestState] = useState<CalculationRequest>(defaults);
  const calculationSequence = useRef(new CalculationRevision());
  const setRequest = useCallback(
    (value: SetStateAction<CalculationRequest>) => {
      calculationSequence.current.invalidate();
  ...
  const selectCity = (place: Place) => {
    ...
    setNotice(
      `${place.name} selected. Timezone set to ${place.timezone}. Your timetable updates automatically.`,
    );
  };

  const apply = useCallback(async (r: CalculationRequest, save = true) => {
    const sequence = calculationSequence.current.snapshot();
    setBusy(true);
    setError('');
    try {
      const [result, p] = await Promise.all([calculate(r), listProfiles()]);
      if (!calculationSequence.current.accepts(sequence)) return result;
      setProfiles(p);
      setReady(true);
      setDay(result);
      setCalendar(null);
      ...
      if (calculationSequence.current.accepts(sequence)) setError(String(e));
      throw e;
    } finally {
      if (calculationSequence.current.accepts(sequence)) setBusy(false);
    }
  }, []);
  useEffect(() => {
    if (!ready || locationBusy) return;
    if (day && sameRequest(request, day.request)) {
      setBusy(false);
      return;
    }
    setBusy(false);
    setError('');
    // Wait briefly for a date/number edit to finish; the core validates the complete request.
    if (!completeConfiguration(request)) return;
    const timer = setTimeout(() => {
      void apply(request).catch(() => {});
    }, 300);
    return () => clearTimeout(timer);
  }, [request, ready, locationBusy, apply]);
  useEffect(() => {
    let active = true;
    ...
    try {
      ...
      setNotice(
        match.candidates.length > 1
          ...
          : `Location and timezone (${match.timezone}) detected. Your timetable updates automatically.`,
      );
    } catch (e) {
      if (sequence !== locationSequence.current) return;
  ...
  const makeCalendar = async () => {
    ...
    try {
      ...
      const result = await calculateRange(
        { ...day.request, date: start },
        days,
      );
      if (calculationSequence.current.accepts(sequence)) setCalendar(result);
    } catch (e) {
  ...
  return (
    <Localized>
      <main className="shell">
        ...
        <div className="workspace">
          ...
          >
            ...
            <div className="detected-timezone">
              ...
              <small>
                Set automatically when you select a city or use GPS.
              </small>
            </div>
            <details
            ...
            <details className="advanced">
              ...
              <div>
                ...
                />
                <p className="field-note">
                  Offsets tune the timetable. Raw sunset and dependent rules
                  stay unchanged.
                </p>
            ...
            <p className="field-note">
              Changes apply automatically. Settings are saved on this device
              after calculation. Your coordinates stay here.
            </p>
            <Button
              type="button"
              variant="ghost"
          ...
          <div className="results-column">
            ...
            {dirty && (
              <p className="notice" role="status">
                Settings changed. The timetable updates automatically when the
              ...
              <Tabs value={mode} onValueChange={setMode}>
                              ...
                              .map((r) =>
                                r
                                  .map(
                                    (c) =>
                                      '"' +
                                      translate(c, language).replaceAll(
                                        '"',
                                        '""',
                                      ) +
                                      '"',
                        ...
                        <div className="table-scroll">
                          <table>
                            <caption className="sr-only">
                              ...
                              al-Qura Hijri dates, sunrise, solar noon and
                              sunset; asterisk marks estimated times.
                            </caption>
                            <thead>
                              <tr>
                ...
                {mode === 'research' && day && (
                  <TabsContent value="research" className="research-view">
                    ...
                    <p>
                      Each result retains its raw instant, adjustment history
                      and estimation basis.
                    </p>
                    <dl>
                      <dt>Method</dt>
            ...
            {day && (
              <section className="method-note">
                ...
                <p>
                  ...
                  {
                    profiles?.fiqh.find(
                      (f) => f.id === day.request.profiles.fiqh,
                    )?.asr_factor
                  }
                  .{' '}
        ...
        <footer>
          <span>
            PrayerTime <span className="footer-dot">·</span> Research in the
            open
          </span>
